# frozen_string_literal: true

module Clans
  class WarriorsController < ApplicationController
    def show
      render json: serializer.new(warrior)
    end

    def index
      warriors = clan.warriors
      if params.key?(:alive)
        if params[:alive].to_i.zero?
          render json: serializer.new(warriors.dead)
        else
          render json: serializer.new(warriors.alive)
        end
      else
        render json: serializer.new(warriors)
      end
    end

    def create
      warrior = clan.warriors.create!(warrior_params)

      render json: serializer.new(warrior), status: 201
    end

    def update
      warrior.update!(warrior_params)

      render json: serializer.new(warrior)
    end

    def destroy
      warrior.destroy!
    end

    private

    def clan
      @clan ||= Clan.find(params[:clan_id])
    end

    def warrior
      @warrior ||= Warrior.find_by!(id: params[:id], clan_id: params[:clan_id])
    end

    def warrior_params
      params.permit(:name, :death_date, :armor_quality, :number_of_battles, :join_date)
    end

    def serializer
      WarriorSerializer
    end
  end
end
